...
export default function Sidebar({ forceOpen, onToggle }) {
  ...
  return (
    <>
      ...
      >
        <div className="w-72 h-full bg-[#0f172a] shadow-2xl flex flex-col">
                ...
                <div key={pageName}>
                  ...
                  >
                    <span className={`h-px w-4 rounded-full flex-shrink-0 transition-colors duration-200 ${
                      isCurrentPage ? 'bg-blue-400' : 'bg-white/20'
                    }`} />
                    {pageName}
                  </button>
                    ...
                    {pageData.components.map((component) => {
                      ...
                      return (
                        <button
                          ...
                          className="w-full text-left flex items-center gap-2.5 px-2 py-1.5 rounded-lg text-xs text-slate-500 hover:text-slate-200 hover:bg-white/5 transition-all duration-150"
                        >
                          {/* Viewed indicator */}
                          {viewed ? (
                            ...
                            <span className="flex-shrink-0 w-4 h-4 rounded-full border border-white/10" />
                          )}
                          {component.title}
                        </button>
                      );
          ...
          <div className="px-5 py-4 border-t border-white/10">
            <p className="text-xs text-slate-600 tracking-wide">BEĨ Educação © 2026</p>
          </div>
      ...
      <button
        ...
        className={`fixed top-1/2 -translate-y-1/2 z-50 transition-all duration-300 ease-in-out
          ...
          bg-[#0f172a] hover:bg-[#1e293b] text-slate-300 hover:text-white
